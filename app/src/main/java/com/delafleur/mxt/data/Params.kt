...
sealed class Params {
    data class GaussianBlurParams(val kSize: Double, val sigmaX: Double, val sigmaY: Double) :
        Params()
    data class BlobParam(var nuts: String) :Params()
    ...
    data class ErodeParams(val kSize: Int, val iterations: Int) : Params()
    data class DilateParams(val kSize: Int, val iterations: Int) : Params()
}